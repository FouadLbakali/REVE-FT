...
Pipeline (each stage propagates its best to the next, per seed):
  1. linear   sweep --lr   (selection on val_mean over seeds)
  2. for each seed: retrain LP with the winning config -> ckpts/lp_<ds>_s{seed}.pt
  3. global   sweep --lora-lr  (each seed loads its own LP ckpt)
  4. for each seed: retrain GL with the winning config -> ckpts/gl_<ds>_s{seed}/
  5. stacked  sweep --lora-lr  (each seed loads its own GL adapters)
  6. subject_specific sweep --lora-lr (each seed loads its own LP)
...
def retrain_lp_per_seed(dataset, seeds, lr, epochs, num_subjects, ckpt_pattern):
    for seed in seeds:
...
def retrain_gl_per_seed(dataset, seeds, lora_lr, gl_epochs,
...
def main():
    ...
    retrain_lp_per_seed(args.dataset, seeds, best_lp["value"],
                        args.lp_epochs, args.num_subjects, lp_ckpt_pat)

    # 3) Global -----------------------------------------------------------
    _, best_gl = sweep(
        mode="global", dataset=args.dataset, param="--lora-lr",
    ...
    retrain_gl_per_seed(args.dataset, seeds, best_gl["value"],
                        args.gl_epochs, args.num_subjects, lp_ckpt_pat, gl_ckpt_pat)

    # 5) Stacked ----------------------------------------------------------
    _, best_st = sweep(
        mode="stacked", dataset=args.dataset, param="--lora-lr",
    ...
    # 6) Subject-specific -------------------------------------------------
    _, best_ss = sweep(
        mode="subject_specific", dataset=args.dataset, param="--lora-lr",
