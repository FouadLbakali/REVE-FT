...
def train_stage(  # noqa: C901, PLR0912, PLR0913, PLR0915
    ...
    for epoch in range(n_epochs):
        ...
        if idr_torch.is_master:
            val_metrics = test(og_model, val_loader, device=device, binary=False)
        else:
        ...
        if best_val < val_balanced_acc:
            ...
            if idr_torch.is_master:
                test_metrics = test(og_model, test_loader, device=device, binary=False)
            else:
...
def train_one_epoch(  # noqa: PLR0913
    ...
    dtype = config.get("torch_dtype", "fp32")
...
def test(model: ReveClassifier, test_loader: torch.utils.data.DataLoader, device="cuda", binary=False):
    score, count = 0, 0
    model.eval()
    y_decisions = []
    y_targets = []
    y_probs = []
    ctx = (
        torch.amp.autocast(device_type="cuda", dtype=model.parameters().__next__().dtype)
        if device == "cuda"
        else nullcontext()
    )
