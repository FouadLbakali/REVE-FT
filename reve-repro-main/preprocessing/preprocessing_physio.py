...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--root",
        type=str,
        default=PATHS["physionet"]["raw"],
        help=f"Root directory of the PhysioNet dataset, defaults to {PATHS['physionet']['raw']}",
    )
    parser.add_argument(
        "--processed",
        type=str,
        default=PATHS["physionet"]["processed"],
        help=f"Directory to save the processed data, defaults to {PATHS['physionet']['processed']}",
    )
    parser.add_argument(
        "--type",
        type=str,
    ...
    for files_key, files_list in files_dict.items():
        for file in files_list:
            for task in tasks:
                ...
                epochs = mne.Epochs(
                    raw, events_from_annot, event_dict, tmin=0, tmax=4.0 - 1.0 / raw.info["sfreq"], baseline=None, preload=True
                )
