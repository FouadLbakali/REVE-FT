...
def loaders(idx,df_dataset,df_dt,path,dataset_name,EA,clip,batch_size,session=True):    
    if dataset_name == 'TUEV':
        ...
        test_loader = DataLoader(test_dataset, batch_size=256, pin_memory=True, shuffle=False, num_workers=16, persistent_workers=True,drop_last=False)
        ...
        train_loader = DataLoader(train_dataset, batch_size=256, pin_memory=True, shuffle=True, num_workers=16, persistent_workers=True,drop_last=True)

        # val_dataset = TUEV(val_files, path_train,pos)
        # val_loader = DataLoader(val_dataset, batch_size=256, pin_memory=True, shuffle=False, num_workers=16, persistent_workers=True,drop_last=True)
    ...
    if dataset_name == 'PhysionetMI_LR':
        n_s = df_dataset.subject.max()
        n_val = max(int(n_s/10),1)
        n_test = n_val*2
        sub_list = np.arange(n_s)
        if idx==0:
            train_set = sub_list[n_val+n_test:]
            val_set = sub_list[:n_val]
            test_set = sub_list[n_val:n_test]
        if idx==1:
            train_set = sub_list[:-n_test]
            val_set = sub_list[-n_val:]
            test_set = sub_list[-n_test:-n_val]
    elif dataset_name == 'BNCI2014_001':
        train_set = [0,1,2,3,4]
        val_set = [5,6]
        test_set = [7,8] 
    elif dataset_name == 'PhysionetMI':
        train_set = np.arange(0,70).tolist()
        val_set = np.arange(70,90).tolist()
        test_set = np.arange(90,110).tolist()
        
    if dataset_name in ['TUAB','TUEV']:
        batch_size = 1024        
        ...
        batch_size = 256
        df_train = df_dataset[df_dataset['split']=='train']
        df_train = df_train[~df_train['subject'].isin([3,56,23,30])]

        df_val = df_dataset[df_dataset['split']=='val']
        print(path)
        df_test = df_dataset[df_dataset['split']=='test']
        
        train_segs = [str(t)+'_-_'+str(se) for t,se in zip(df_train['trials'].values,df_train['subject'].values)]
        val_segs = [str(t)+'_-_'+str(se) for t,se in zip(df_val['trials'].values,df_val['subject'].values)]
        test_segs = [str(t)+'_-_'+str(se) for t,se in zip(df_test['trials'].values,df_test['subject'].values)]
        
        print(len(df_train),len(df_val),len(df_test))

        
    else:
        df_train = df_dataset[df_dataset['subject'].isin(train_set)]
        df_val = df_dataset[df_dataset['subject'].isin(val_set)]
        df_test = df_dataset[df_dataset['subject'].isin(test_set)]
        train_segs = [str(t)+'_-_'+str(s)+'_-_'+str(se) for t,s,se in zip(df_train['trial'].values,df_train['subject'].values,df_train['session'].values)]
        val_segs = [str(t)+'_-_'+str(s)+'_-_'+str(se) for t,s,se in zip(df_val['trial'].values,df_val['subject'].values,df_val['session'].values)]
        test_segs = [str(t)+'_-_'+str(s)+'_-_'+str(se) for t,s,se in zip(df_test['trial'].values,df_test['subject'].values,df_test['session'].values)]
        
    train_dataset = EEGDTDataset(train_segs,df_dt,df_dataset,path,dataset_name,clip=clip,EA=EA,session=session)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, pin_memory=True,shuffle = True, num_workers=16,persistent_workers=True)

    val_dataset = EEGDTDataset(val_segs, df_dt, df_dataset, path, dataset_name, clip=clip, EA=EA,session=session)
    val_loader = DataLoader(val_dataset, batch_size=256, pin_memory=True, shuffle=False, num_workers=16, persistent_workers=True,drop_last=False)
...
def train_test(df_dataset,df_dt,path,dataset_name,model_state_dict,args):
    ...
    if args.ft_dataset in ['TUAB','TUEV']:
        ...
        args.patience_lp = 2
        args.patience_ft = 2
        args.ES_patience_lp = 5
        args.ES_patience_ft = 5
