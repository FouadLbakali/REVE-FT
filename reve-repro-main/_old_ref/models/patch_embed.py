...
class Learnable4DPE(nn.Module):
    def __init__(self, embed_dim: int, positions: torch.Tensor, n_timesteps: int):
        ...
        self.register_buffer("time_indices", torch.arange(n_timesteps))
    ...
    def _convert_positions(self, pos: torch.Tensor, eps: float = 1e-5):
        ...
        with torch.autocast("cuda" if pos.is_cuda else "cpu"):
            indices = torch.cdist(pos, self.positions.to(pos)).argmin(dim=-1)  # cdist breaks with half precision
    ...
    def forward(self, pos):
        ...
        temporal_pe = self.temporal_pe(self.time_indices)  # T, E

        spatial_pe = spatial_pe.unsqueeze(2).expand(-1, -1, self.n_timesteps, -1)
